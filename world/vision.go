...
func NewVision2(size Pt) *Vision2 {
	var v Vision2
	v.blockSize = I(1000)
...
func (v *Vision2) tileToWorldPos(pt Pt) Pt {
	half := v.blockSize.DivBy(TWO)
	offset := Pt{half, half}
	return pt.Times(v.blockSize).Plus(offset)
}

func (v *Vision2) worldPosToTile(pt Pt) Pt {
	return pt.DivBy(v.blockSize)
}

func (v *Vision2) computeRelativeRelevantPts(dif Pt) (pts []Pt) {
...
func (v *Vision2) isPathClear(start, end Pt, obstacles MatBool) bool {
	if start == end {
		return true
	}
...
func (v *Vision2) Compute(start Pt, obstacles MatBool) (attackableTiles MatBool) {
	attackableTiles = NewMatBool(obstacles.Size())
