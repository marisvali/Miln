...
type LineSquareIntersectionCache struct {
	intersections map[LineSquare]bool
}

func NewLineSquareIntersectionCache() (c LineSquareIntersectionCache) {
	c.intersections = map[LineSquare]bool{}
	return c
}

func (c *LineSquareIntersectionCache) Intersection(l Line, s Square) bool {
	ls := LineSquare{l, s}
	if val, ok := c.intersections[ls]; ok {
		return val
	}

	val, _ := LineSquareIntersection(l, s)
	c.intersections[ls] = val
	return val
}
...
func (a *AttackableTiles3) computeRelativeRelevantPts(dif Pt) (pts []Pt) {
	...
	for y := start.X; y.Leq(end.Y); y.Inc() {
		for x := start.Y; x.Leq(end.X); x.Inc() {
			...
			intersects := a.lineSquareIntersectionCache.Intersection(l, square)
			if intersects {
				pts = append(pts, pt)
			}
		}
